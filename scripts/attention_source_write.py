"""Closed-form source writes into attention output projections.

This is the attention-side analogue of the TSOC MLP down-projection write. It
does not copy attention probabilities. It writes into attention `o_proj`, using
the attention output projection input as the trigger key and the teacher
attention-output delta as the target residual contribution.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output", default="runs/attention_source_write")
    ...
    parser.add_argument("--sequential-replay", action="store_true")
...
def main() -> None:
    ...
    wrappers = install_additive_attention_memory(model, args.layers, memory_dtype=torch.float32)
    ...
    for paper_idx, (domain, heldout_questions) in enumerate(zip(domains, eval_sets)):
        ...
        if not args.sequential_replay:
            null_attention = capture_attention_io(
                model,
                tokenizer,
                null_prompts,
                args.layers,
            ...
            key_attention = capture_attention_io(
                model,
                tokenizer,
                key_prompts,
                args.layers,
        ...
        guard_captures = capture_attention_io(
            model,
            tokenizer,
            guard_prompts,
            args.layers,
        ...
        for layer_idx in args.layers:
            if args.sequential_replay:
                ...
                trace_keys = current_attention[layer_idx].keys
                targets = full_attention[layer_idx].outputs.float() - current_attention[layer_idx].outputs.float()
            else:
                trace_keys = key_attention[layer_idx].keys
                targets = full_attention[layer_idx].outputs.float() - null_attention[layer_idx].outputs.float()
    ...
    print(f"Wrote attention write metrics to {metrics_path}")
    print(f"Wrote attention write update stats to {updates_path}")
    if args.retention_eval:
        print(f"Wrote attention write retention metrics to {retention_path}")
